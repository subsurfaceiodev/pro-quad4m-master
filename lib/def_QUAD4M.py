...
def common_def():
    ...
    common_def_dict[
        "out_folder"] = 'directory where outputs are saved (default: "var"); herein a directory will be created if not existent, whose name is specified in the JSON-file (see "json" positional argument above-mentioned) within the field "job_folder" of "modelling_parameters" section'
...
def is_valid_dir(arg):
    if not os_path.isdir(arg):
        raise argparse.ArgumentTypeError(
            "Folder '%s' does not exist! Please create it before proceeding or furnish a valid folder using the specific command-line argument." % arg)
    else:
        return arg
...
def add_tolerance(strata_s, tolerance_s, min_s, max_s):
    for c_strata_s in strata_s:
        ...
        if cn_strata_s >= min_s:
            c_strata_s[1] = cn_strata_s
        else:
            c_strata_s[1] = min_s
        cm_strata_s = c_strata_s[2] + tolerance_s
        if cm_strata_s <= max_s:
            c_strata_s[2] = cm_strata_s
        else:
            c_strata_s[2] = max_s
    return strata_s
...
def find_intervals(strata_s, min_val, max_val, strata, min_dim):
    ...
    for c_ndx in range(strata):
        ...
        if not c_ndx:
            ...
            c_poi_rep_for = strata_st[c_ndx - 1][2]
...
def adjust_intervals(c_y_lines, c_pois_def, def_coe, dist_threshold, e_exp, i_exp):
    ...
    for s_ndx in range(len(s_diff) // 2):
        g_ndx = -(s_ndx + 1)
    ...
    for k_poi in c_pois_def:
        ...
        min_ndx = np.argmin([abs(x - k_poi) for x in c_y_lines])
        ...
        a_diff = list(map(lambda x, y: diff_k_poi * ((x) ** i_exp + (y) ** i_exp), c_diff, b_diff))
        d_diff = list(map(lambda x, y: x + y, a_diff, d_diff))

    d_diff = list(map(lambda x, y: x * y, d_diff, s_diff))
    c_y_lines = list(map(lambda x, y: x + y, c_y_lines, d_diff))
...
def adjust_line(h_dist, h_dist_threshold, h_arr_line, h_diff, o_exp):
    if h_dist < h_dist_threshold:
        ...
        h_arr_line = list(map(lambda x, y: x + y * d_mp_val_ts, h_arr_line, h_diff))
    return h_arr_line
...
def adjust_all_intervals(w_arr_lines, c_dir, v_arr_lines,
    ...
    for v_ndx in range(len(w_arr_lines)):
        ...
        if np.max(np.abs(w_diff)):
            ...
            v_v_mean = np.mean(list(zip(*wv_arr_lines[v_ndx]))[c_dir_ndx])
            for j_ndx in range(1, len(w_arr_lines) // 5):  # [1, 2, 3, 4, ...]
                d_ndx = v_ndx - j_ndx
                if d_ndx in range(len(w_arr_lines)):
                    d_c_dist = abs(v_v_mean - np.mean(list(zip(*wv_arr_lines[d_ndx]))[c_dir_ndx]))
                    w_arr_lines[d_ndx] = \
                        adjust_line(d_c_dist, dist_threshold, w_arr_lines[d_ndx], w_diff, orth_exp)
                e_ndx = v_ndx + j_ndx
                if e_ndx in range(len(w_arr_lines)):
                    e_c_dist = abs(np.mean(list(zip(*wv_arr_lines[e_ndx]))[c_dir_ndx]) - v_v_mean)
                    w_arr_lines[e_ndx] = \
...
def retrieve_poi_s(c_dir, ab_arr_line, input_dict, strata, json_elem_dict):
    ...
    for stratum_ndx in [x + 1 for x in range(strata)]:
        ...
        attr = c_dir.lower()
        if c_ls_inter.geom_type in ('LineString', 'MultiLineString'):
            boundary_points = c_ls_inter.boundary
            c_pois.extend(getattr(point, attr) for point in boundary_points.geoms)
        elif c_ls_inter.geom_type == 'Point':
            c_pois.append(getattr(c_ls_inter, attr))
        elif hasattr(c_ls_inter, 'geoms'):  # Handle GeometryCollection or other multi-geometries
            for geom in c_ls_inter.geoms:
                if geom.geom_type == 'Point':
                    c_pois.append(getattr(geom, attr))
                elif geom.geom_type == 'LineString':
                    boundary_points = geom.boundary
                    c_pois.extend(getattr(point, attr) for point in boundary_points.geoms)
    return c_pois
...
def retrieve_pois_ndx(a_arr_lines, a_poi_def):
    ...
    for b_ndx in range(len(a_arr_lines)):
        ...
        for b_poi in c_pois_def:
            b_poi_ndx = np.argmin([abs(x - b_poi) for x in a_arr_lines[b_ndx]])
...
def add_element(c_node_a, c_node_b, c_node_c, c_node_d, strata, json_elem_dict, input_dict,
    ...
    if stratum_fx == None:  # 'None' (find soil-identifier) or soil: '0' (no soil) or '1', '2', etc.
        ...
        if not c_ele_st.is_ccw:
            ccw_continue = False
            print("WARNING: element " + str(c_node_a[0]) + "-" + str(c_node_b[0]) + "-" + str(c_node_c[0]) + "-" + str(
                c_node_d[0]) + " not in counter-clockwise order")
    ...
    if stratum_fx is not None and stratum_fx > 0 or c_ins == True:
        ...
        c_val_DENS = val_uncentanty(input_dict[c_stratum][json_elem_dict["prem_dns"]],
                                    json_elem_dict["etc_va"], json_elem_dict["etc_mv"], json_elem_dict["etc_uc"],
                                    json_elem_dict["etc_ln"])
        ...
        element_lines_write.append([c_node_a[0], c_node_b[0], c_node_c[0], c_node_d[0],
                                    stratum_ndx, c_val_DENS, c_val___PO, c_val__GMX, c_val__GMX * G_start_percent,
                                    c_val___XL])
...
def add_markers(c_plt, no_array, marker_symbol, legend_description, c_color,
    ...
    if no_array:
        ...
        # ~ add_legend = False
        # ~ else: c_label = None
        no_array_t = list(map(list, list(zip(*no_array))))
...
def post_plot(plt, plt_ax, no_bc_array_1, no_bc_array_2, no_bc_array_3, no_bc_array_4,
    ...
    plt, bc_leg_1 = add_markers(c_plt=plt, no_array=no_bc_array_1, marker_symbol=">",
                                legend_description="horizontal motion applied, free in vertical direction",
                                c_color=c_col, c_markersize=c_l_width)
    plt, bc_leg_2 = add_markers(c_plt=plt, no_array=no_bc_array_2, marker_symbol="^",
                                legend_description="vertical motion applied, free in horizontal direction",
                                c_color=c_col, c_markersize=c_l_width)
    plt, bc_leg_3 = add_markers(c_plt=plt, no_array=no_bc_array_3, marker_symbol="P",
                                legend_description="horizontal and vertical motion applied", c_color=c_col,
                                c_markersize=c_l_width)
    ...
    plt, out_leg_3 = add_markers(c_plt=plt, no_array=no_out_array_3, marker_symbol="P",
                                 legend_description="horizontal and vertical output", c_color=c_col,
                                 c_markersize=c_l_width)

    all_leg_2 = [bc_leg_1, bc_leg_2, bc_leg_3, bc_leg_4, out_leg_1, out_leg_2, out_leg_3]
    all_leg_2 = [_f for _f in all_leg_2 if _f]
...
def par_plot(plt, plt_ax, input_dict, json_elem_dict, lprint_str, xy_mtri, xy_triang,
    ...
    if save_png: figure.savefig((par_name[:-3] + 'png').replace(outfold, png_outfold), format='png', dpi=600,
                                bbox_inches='tight')
    # #############################
    return c_s
...
def KonnoOhmachi(SP_in, freq, b):
    ...
    for i in range(1, l_SP):
        ...
        ndxs = list(range(1, i)) + list(range(i + 1, l_SP))
...
def mesh_ds(element_lines_H, element_lines_H_mask, ref_soil, min_F_nodes, json_elem_dict,
    ...
    mm_range = list(range(len_H - 1))
    jj_range = list(range(len_H0 - 1))
    strata = int(input_dict[json_elem_dict["modp"]][json_elem_dict["modp_str"]])
    for mm in mm_range:
        ...
        for jj in jj_range:
            ...
            if start_hs:
                ...
                jn = jj
                nn_range = list(range(min_F_nodes - int(1)))
                for nn in nn_range:
                    jm = jn
            ...
            if not nn_cont:
                ...
                nn_cont += 1
                if jj == 0:
                    p_elm = 'Q'  # i.e. on first endpoint of each polygonal-chain
                else:
                    p_elm = '1'  # ('hs1', 'hd1') or ('vs1', 'vd1')
            else:
                ...
                if cond_H or 0 < nn_cont < min_F_nodes - int(2):
                    ...
                    nn_cont = -1
                    if jk == dl_H0:
                        p_elm = 'Q'  # i.e. on last endpoint of each polygonal-chain
                    else:
                        p_elm = '2'  # ('hs2', 'hd2') or ('vs2', 'vd2')
...
def add_fake_elements(element_lines_H, diff_x_lines, diff_y_lines):
    ...
    element_lines_H = list(map(list, list(zip(*element_lines_H))))
...
def element_lines_numpy(element_lines_H):
    element_lines_H_NEW = np.full((len(element_lines_H), len(element_lines_H[0])), np.nan, dtype=object)
