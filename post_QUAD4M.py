...
p = argparse.ArgumentParser(
    description='description: Generate 1) a serie of graphical-outputs in SVG format (Scalable Vector Graphics) of the QUAD4M outputs, and 2) a file, whose extension is "sd", containing the QUAD4M acceleration output in terms of displacement response spectra.')

p.add_argument("json", action="store", type=is_valid_file, help=common_def_dict['json'])
p.add_argument("Q4O", action="store", type=is_valid_file,
               help='QUAD4M main output file (denoted by extension "Q4O" in QUAD4M user''s manual)')
p.add_argument("Q4A", action="store", type=is_valid_file,
               help='QUAD4M acceleration output file (denoted by extension "Q4A" in QUAD4M user''s manual)')
...
if spec_outfold != d_var_O:
    print(
        'ERROR: directory "' + spec_outfold + '" specified in "' + infile + '" (field "job_folder") does not correspond to directory "' + d_var_O + '" where file "' + infile_O + '" is located')
    print('NOTE: to avoid this error, consider to pass "' + infile_O.replace(d_var_O,
                                                                             spec_outfold) + '" instead of "' + infile_O + '"')
    sys_exit(1)
d_var_A = os_path.split(os_path.split(infile_A)[0])[-1]
if spec_outfold != d_var_A:
    print(
        'ERROR: directory "' + spec_outfold + '" specified in "' + infile + '" (field "job_folder") does not correspond to directory "' + d_var_A + '" where file "' + infile_A + '" is located')
    print('NOTE: to avoid this error, consider to pass "' + infile_A.replace(d_var_A,
                                                                             spec_outfold) + '" instead of "' + infile_A + '"')
    sys_exit(1)
d_var_B = os_path.split(os_path.split(infile_B)[0])[-1]
if spec_outfold != d_var_B:
    print(
        'ERROR: directory "' + spec_outfold + '" specified in "' + infile + '" (field "job_folder") does not correspond to directory "' + d_var_B + '" where file "' + infile_B + '" is located')
    print('NOTE: to avoid this error, consider to pass "' + infile_B.replace(d_var_B,
                                                                             spec_outfold) + '" instead of "' + infile_B + '"')
    sys_exit(1)
...
tol_nod = 0.2
s_B = io_StringIO("" + data_B)
mat_B = np.genfromtxt(fname=s_B, dtype='f8,f8,f8,f8', delimiter=(10, 10, 10, 10),
                      names=('cs00', 'cs01', 'cs10', 'cs11'))
c_s_B = []
...
xy_triang_ndxs_O = []
mat_pars_O = []
for c_k in range(len(mat_elems_O)):
...
for cc__X in c__X_bll:
    [c__X_nodes_surf.append(x) for x in surf_nodes if
     np.allclose([x[0], x[1]], [cc__X[0], cc__X[1]], rtol=.0, atol=tol_nod)]
...
for cc__X in c__X_all:
    [c__X_nodes_all.append(x) for x in mat_node_O if
     np.allclose([x[1], x[2]], [cc__X[0], cc__X[1]], rtol=.0, atol=tol_nod)]
...
for cc__Y in c__Y_bll:
    [c__Y_nodes_surf.append(x) for x in surf_nodes if
     np.allclose([x[0], x[1]], [cc__Y[0], cc__Y[1]], rtol=.0, atol=tol_nod)]
...
for cc__Y in c__Y_all:
    [c__Y_nodes_all.append(x) for x in mat_node_O if
     np.allclose([x[1], x[2]], [cc__Y[0], cc__Y[1]], rtol=.0, atol=tol_nod)]

# ~ ##############################

c__X_nodes_numb = list(map(str, list(map(list, list(zip(*c__X_nodes_all))))[0]))
c__Y_nodes_numb = list(map(str, list(map(list, list(zip(*c__Y_nodes_all))))[0]))
...
s_acc_A = io_StringIO("" + data_A)

for i, hdr_acc_A in enumerate(s_acc_A):
    if i == 2: break
h_hdr_acc_A = io_StringIO("" + hdr_acc_A)
...
for t_name in acc_names[1:]:
    ...
    c_PSV = fH(TH)
    par_dict['HI'][t_name] = np.trapezoid(c_PSV, x=TH)
...
xy_triang_STRESS = list(map(list, list(zip(*mat_pars_O))))[3]
xy_triang_STRAIN = list(map(list, list(zip(*mat_pars_O))))[4]
xy_triang_STRAIN = [0.0001 if not v else v for v in xy_triang_STRAIN]
xy_triang_XACC = list(map(list, list(zip(*mat_pars_O))))[6]
xy_triang_YACC = list(map(list, list(zip(*mat_pars_O))))[7]
...
c__Y_par_w = list(map(list, list(zip(*c__Y_par))))
c__X_par = []
...
c__X_par_w = list(map(list, list(zip(*c__X_par))))
...
plt_leg0 = plt_ax[0].legend(fontsize=leg_fontsize, loc=1, facecolor=[1., 1., 1.], edgecolor=[1., 1., 1.],
...
plt, plt_ax[1] = finalize_plot(plt, plt_ax[1], ax_linewidth, ax_fontsize, box='box', plt_box=True)
...
plt_ax[1].annotate('', xy=(0.02, 0.55), xycoords='figure fraction', xytext=(0.02, 0.45),
                   arrowprops=dict(arrowstyle="-", color='w'))
plt_ax[1].annotate('', xy=(0.98, 0.55), xycoords='figure fraction', xytext=(0.98, 0.45),
                   arrowprops=dict(arrowstyle="-", color='w'))
...
for c_val in range(2, 8):
    ...
    if save_png: figure.savefig((par_names[c_val_n][0][:-3] + 'png').replace(outfold, png_outfold), format='png',
                                dpi=600, bbox_inches='tight')

# ~ ##############################

if input_dict[json_elem_dict["pltp"]][json_elem_dict["pltp_bwc"]]:
    cmap = plt.get_cmap('Greys')
else:
    cmap = plt.get_cmap('jet')
...
for h__ndx in range(1, len(mat_SA_A[acc_names[0]])):
    vals_X.append(
        mat_SA_A[acc_names[0]][h__ndx] - (mat_SA_A[acc_names[0]][h__ndx] - mat_SA_A[acc_names[0]][h__ndx - 1]) / 2)
...
c_bar_ticklabels = [c_fmt_punct.format(x) for x in c_bar_ticks]
...
if save_png: figure.savefig((par_names[-1][0][:-3] + 'png').replace(outfold, png_outfold), format='png', dpi=600,
                            bbox_inches='tight')
...
for h__ndx in range(1, len(mat_K_A[acc_names[0]])):
    vals_X.append(
        mat_K_A[acc_names[0]][h__ndx] - (mat_K_A[acc_names[0]][h__ndx] - mat_K_A[acc_names[0]][h__ndx - 1]) / 2)
...
xy_mtri_SA = mtri.Triangulation(vals_Yd, vals_Xd, triangles=vals_Kd)
tpc = plt_ax[0].tripcolor(xy_mtri_SA, vals_Zd, shading='flat', cmap=cmap,
                          norm=clr.LogNorm(vmin=c_minVal, vmax=c_maxVal))
...
if save_png: figure.savefig((out_fft[:-3] + 'png').replace(outfold, png_outfold), format='png', dpi=600,
                            bbox_inches='tight')
...
c_col = [0.50] * 3
for c__ndx in range(len_c__Y):
    if np.allclose(c_col, [0.50] * 3):
        c_col = [0.00] * 3
    elif np.allclose(c_col, [0.00] * 3):
        c_col = [0.75] * 3
    elif np.allclose(c_col, [0.75] * 3):
        c_col = [0.25] * 3
    elif np.allclose(c_col, [0.25] * 3):
        c_col = [0.50] * 3
    cc_str = 'Node' + str(c__Y_nodes[c__ndx][0]) + 'Y'
    vals_Y = c__Y_nodes[c__ndx][1]
    plt_ax[0].plot(mat_acc_A[cc_str] * scale_v + vals_Y, mat_acc_A[acc_names[0]], linestyle='-', marker='', color=c_col,
...
if save_png: figure.savefig((par_names[-2][0][:-3] + 'png').replace(outfold, png_outfold), format='png', dpi=600,
                            bbox_inches='tight')
...
c__X_nodes_surface = list(map(str, list(map(list, list(zip(*c__X_nodes))))[0]))
c__Y_nodes_surface = list(map(str, list(map(list, list(zip(*c__Y_nodes))))[0]))
...
for ix in range(len(c__X_nodes_numb)):
    ...
    if c__X_nodes_numb[ix] in c__X_nodes_surface:
        str_S = 'Y'
    else:
        str_S = 'N'
...
for iy in range(len(c__Y_nodes_numb)):
    ...
    if c__Y_nodes_numb[iy] in c__Y_nodes_surface:
        str_S = 'Y'
    else:
        str_S = 'N'
    N_fid.write('{:>18s}'.format(str_S + '\n'))
