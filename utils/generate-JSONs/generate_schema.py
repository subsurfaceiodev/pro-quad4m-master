...
boundaries_schema_inner_dict = {'horizontal': pos_val_schema_dict, 'vertical-left': pos_val_schema_dict,
                                'vertical-right': pos_val_schema_dict}
...
model_G_GMX_XL_schema_dict = core_schema_dict(
    ['model_G_GMX_XL_tf', 'depth_of_water_table_m', 'coef_of_lateral_earth_pressure_K0', 'mean_effective_stress_atm',
     'plasticity_index_PI_percent', 'over_consolidation_ratio_OCR', 'XL_uncentanty'], model_G_GMX_XL_schema_inner_dict)
...
rearrange_nodes_dict = core_schema_dict(
    ['rearrange_nodes_tf', 'iterations', 'scaling_factor', 'distance_threshold_m', 'vertical_deformation_coefficient',
     'horizontal_deformation_coefficient', 'overall_exponent', 'exponents'], rearrange_nodes_inner_dict)
...
mesh_downsampling_dict = core_schema_dict(
    ['mesh_downsampling_tf', 'minimum_module_length', 'element_shape_factor_multiplier', 'triangular_extremities_tf'],
    mesh_downsampling_inner_dict)
...
mesh_generation_dict = core_schema_dict(
    ['maximum_frequency', 'uniform_initial_grid', 'boundaries_triangular_elements_tf', 'rearrange_nodes',
     'maximum_element_shape_factor', 'mesh_downsampling'], mesh_generation_inner_dict)

# DENS_schema_dict, GMX_schema_dict, G_schema_dict, XL_schema_dict
DENS_schema_inner_dict = \
    val_schema_dict(tm_schema_dict('number', 1), tm_schema_dict(['number', 'null'], 1),
                    tm_schema_dict(['number', 'null'], 0), boolean_dict)
PO_schema_inner_dict = \
    val_schema_dict(tm_schema_dict('number', -1.0, 0.5), tm_schema_dict(['number', 'null'], -1.0, 0.5),
                    tm_schema_dict(['number', 'null'], 0), boolean_dict)
GMX_schema_inner_dict = DENS_schema_inner_dict
XL_schema_inner_dict = \
    val_schema_dict(tm_schema_dict(['number', 'null'], 0.0005, 1.0), tm_schema_dict(['number', 'null'], 0.0005, 1.0),
                    tm_schema_dict(['number', 'null'], 0), boolean_dict)
...
schema_dict_modelling_properties['job_folder'] = {'type': 'string', 'minLength': 1, 'maxLength': 80,
                                                  "pattern": "^[a-zA-Z0-9_]*$"}
...
acceleration_output_dict = core_schema_dict(['X_direction', 'Y_direction', 'XY_direction'],
                                            acceleration_output_inner_dict)
...
schema_dict_waveforms_properties[
    'PRINPUT_period_corresponding_to_maximum_spectral_acceleration_of_horizontal_input_motion_s'] = tm_schema_dict(
    ['number', 'null'], 0.001, 1.0)
...
schema_dict['required'] = ['modelling_parameters', 'mesh_generation', 'waveforms', 'stratum001', 'stratum002',
                           'plot_parameters']
