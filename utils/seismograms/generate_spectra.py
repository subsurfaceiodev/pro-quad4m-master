...
p = argparse.ArgumentParser(description='description: generate corresponding spectra (displacement responses (5% damped), non-smoothed and smoothed Fourier amplitudes) to the input waveform. Smoothing is performed using the Konno and Ohmachi operator with b=40.')
p.add_argument("json", action="store", type=is_valid_file, help=common_def_dict['json'] + ', togheter with waveform parameters')
p.add_argument("acc", action="store", type=is_valid_file, help="single-column acceleration time-history, whose parameters are contained in 'json' file above")
opts = p.parse_args()
