...
p.add_argument("-v", "--version", action="version", version=common_def_dict["version"],
               help=common_def_dict["version_h"])
...
with open(infile, 'r') as input_json:
    try:
        ...
        print(
            'ERROR: file "' + infile + '" does not contain a valid JSON object (e.g. check it through any JSON-validator on the web)."')
        sys_exit(1)
# JSON-schema
...
for c_error in sorted(c_val.iter_errors(input_dict), key=str):
    if list(map(str, list(c_error.absolute_path))):
    ...
    if len(json.dumps(c_error.schema)) < 300:
        ...
        c_error_schema = ''
    c_error_str = 'ERROR: ' + str(c_error.message) + c_error_absolute_path.replace('[', '["').replace(']',
                                                                                                      '"]') + ' (DETAIL: failed validating "' + c_error.validator + '" in schema ["' + '"]["'.join(
        c_error.schema_path) + '"]' + c_error_schema + ')'
...
try:
    ...
    for stratum_ndx in [x + 1 for x in range(strata)]:
        ...
        if not c_stratum in input_dict:
            ...
            c_error_str = 'ERROR: "' + c_stratum + '" is a required property (DETAIL: expected properties are "' + first_stratum + '"..."' + last_stratum + '", since ' + str(
                strata) + ' strata have been declared on instance ["' + json_elem_dict["modp"] + '"]["' + \
                          json_elem_dict["modp_str"] + '"])'
            c_errors.append(c_error_str.replace('\r', '').replace('\n', ''))
        else:
            ...
            for k_nod in st_nod:
                ...
                if sk_nod not in nodal_points:
                    c_error_str = 'ERROR: node "' + sk_nod + '" not found (DETAIL: value "' + sk_nod + '" given on instance ["' + c_stratum + '"]["' + \
                                  json_elem_dict["prem_nod"] + '"] must exists as a key in ["' + c_stratum + '"]["' + \
                                  json_elem_dict["modp_nps"] + '"])'
                    c_errors.append(c_error_str.replace('\r', '').replace('\n', ''))
            ...
            if not input_dict[c_stratum][json_elem_dict["prem_xld"]][json_elem_dict["etc_va"]]:
                if model_G_Gmax_XL_tf:
                    ...
                    c_error_str = 'ERROR: in case "null" is given on instance ["' + c_stratum + '"]["' + json_elem_dict[
                        "prem_xld"] + '"]["' + json_elem_dict[
                                      "etc_va"] + '"], "true" must be given on ["' + c_stratum + '"]["' + \
                                  json_elem_dict["prem_mGX"] + '"]["' + json_elem_dict["prem_mGX_tf"] + '"])'
                    c_errors.append(c_error_str.replace('\r', '').replace('\n', ''))
                ...
                if input_dict[c_stratum][json_elem_dict["prem_soi"]][json_elem_dict["prem_soi_xl"]]:
                    ...
                    if not XL0 == XL:
                        c_error_str = 'ERROR: damping values non-consistent (DETAIL: assuming low-deformation (i.e. low-shear-strain), the first damping value "' + str(
                            float(XL0)) + '" given on instance ["' + c_stratum + '"]["' + json_elem_dict[
                                          "prem_soi"] + '"]["' + json_elem_dict[
                                          "prem_soi_xl"] + '"] must be equal to the value "' + str(
                            float(XL)) + '" given on instance ["' + c_stratum + '"]["' + json_elem_dict[
                                          "prem_xld"] + '"]["' + json_elem_dict["etc_va"] + '"])'
                        c_errors.append(c_error_str.replace('\r', '').replace('\n', ''))
                if input_dict[c_stratum][json_elem_dict["prem_soi"]][json_elem_dict["prem_soi_gg"]]:
                    G_Gmax0 = float(
                        input_dict[c_stratum][json_elem_dict["prem_soi"]][json_elem_dict["prem_soi_gg"]][0][1])
                    if not G_Gmax0 == 1.0:
                        c_error_str = 'ERROR: shear modulus (G) values non-consistent (DETAIL: assuming low-deformation (i.e. low-shear-strain), the first G/Gmax value "' + str(
                            float(G_Gmax0)) + '" given on instance ["' + c_stratum + '"]["' + json_elem_dict[
                                          "prem_soi"] + '"]["' + json_elem_dict[
                                          "prem_soi_gg"] + '"] must be equal to 1.0)'
                        c_errors.append(c_error_str.replace('\r', '').replace('\n', ''))

            min_value_check_fields = [json_elem_dict["prem_xld"], json_elem_dict["prem_gmx"],
                                      json_elem_dict["prem_dns"], json_elem_dict["prem_por"]]
            for check_field in min_value_check_fields:
                ...
                if input_dict[c_stratum][check_field][json_elem_dict["etc_ln"]] \
                        ...
                        and (input_dict[c_stratum][check_field][json_elem_dict["etc_uc"]] < 1.0 or
                             input_dict[c_stratum][check_field][json_elem_dict["etc_uc"]] > 5.0):
...
if os_path.isfile(strata_nam):
    ...
    ans_YN = input("Overwrite all output files? [Y|N] [N]: ")
    if not ans_YN:
        sys_exit(1)
    elif ans_YN[0] in ['y', 'Y']:
        pass
    else:
        sys_exit(1)
...
if input_dict[json_elem_dict["pltp"]][json_elem_dict["pltp_bws"]]:
...
plt_ax.annotate('', xy=(0.02, 0.55), xycoords='figure fraction', xytext=(0.02, 0.45),
                arrowprops=dict(arrowstyle="-", color='w'))
plt_ax.annotate('', xy=(0.98, 0.55), xycoords='figure fraction', xytext=(0.98, 0.45),
                arrowprops=dict(arrowstyle="-", color='w'))
figure = plt.gcf()
figure.savefig(strata_nam, format='svg', bbox_inches='tight')
if save_png: figure.savefig((strata_nam[:-3] + 'png').replace(outfold, png_outfold), format='png', dpi=600,
                            bbox_inches='tight')
# ~ plt.show()
...
for ii, (a_key, a_val) in enumerate(nodal_points.items()):
    ...
    pippo = plt_ax.annotate(str(a_key), (var_np[0] + 2., var_np[1] + 7.), fontsize=ax_fontsize * 5.0 / 8.0, bbox=tbbox,
                            verticalalignment='center')
    pippo.get_bbox_patch().set_boxstyle("square, pad=0.1")
    if ii == 1:
        plt_ax.plot(var_np[0], var_np[1], 'o', linewidth=0.0, markersize=ax_markersize, color='r', label='node')
    else:
        plt_ax.plot(var_np[0], var_np[1], 'o', linewidth=0.0, markersize=ax_markersize, color='r', label=None)
plt_ax.legend(fontsize=leg_fontsize, loc=7, ncol=legend_ncol)
figure = plt.gcf()
figure.savefig(nmodel_nam, format='svg', bbox_inches='tight')
if save_png: figure.savefig((nmodel_nam[:-3] + 'png').replace(outfold, png_outfold), format='png', dpi=600,
                            bbox_inches='tight')
...
if wave_ftl >= wave_ltl:
    print('ERROR: first time-step to be considered must be lower than last! Please chose different values for "' +
          json_elem_dict["wave"] + '" JSON-input block. Consider, in particular, to change "' + json_elem_dict[
              "wave_ftl"] + '" and "' + json_elem_dict["wave_ltl"] + '" input values.')
    sys_exit(1)
if wave_fts >= wave_lts:
    print('ERROR: first time-step to be considered must be lower than last! Please chose different values for "' +
          json_elem_dict["wave"] + '" JSON-input block. Consider, in particular, to change "' + json_elem_dict[
              "wave_fts"] + '" and "' + json_elem_dict["wave_lts"] + '" input values.')
    sys_exit(1)
if wave_ltl > lenwave:
    print(
        'ERROR: last time-step to be considered must be lower than length of whole waveforms! Please chose different values for "' +
        json_elem_dict["wave"] + '" JSON-input block. Consider, in particular, to change "' + json_elem_dict[
            "wave_ltl"] + '" input value.')
    sys_exit(1)
if wave_lts > lenwave:
    print(
        'ERROR: last time-step to be considered must be lower than length of whole waveforms! Please chose different values for "' +
        json_elem_dict["wave"] + '" JSON-input block. Consider, in particular, to change "' + json_elem_dict[
            "wave_lts"] + '" input value.')
    sys_exit(1)
...
for stratum_ndx in [x + 1 for x in range(strata)]:
    ...
    if c_G0_u is None:
        ...
        if input_dict[c_stratum][json_elem_dict["prem_gmx"]][json_elem_dict["etc_ln"]]:
            ...
            c_G0 = c_G0_v - 2.0 * float(c_G0_u)  # case normal (Gaussian)
...
if save_png: figure.savefig((st_me_nam[:-3] + 'png').replace(outfold, png_outfold), format='png', dpi=600,
                            bbox_inches='tight')
...
if not ccw_continue:
    print('ERROR: some elements not in counter-clockwise order, aborting script. Please chose different values for "' +
          json_elem_dict["mesh"] + '" JSON-input block. Consider, in particular, to change "' + json_elem_dict[
              "mesh_ran_at"] + '" and "' + json_elem_dict["mesh_ran_dc"] + '" input values.')
    sys_exit(1)
...
for stratum_ndx in [x + 1 for x in range(strata)]:
    ...
    if model_G_Gmax_XL_tf:
        if print_wt:
            ...
            print_wt = False
        depth_of_water_table_m = float(
            input_dict[c_stratum][json_elem_dict["prem_mGX"]][json_elem_dict["prem_mGX_dwt"]])
        ...
        if not st_ES:
            ...
            if triang_elms_wei:
                ...
                for mm in range(len_H):
                    for jj in range(len_H0):
                        ...
                        if element_lines_H[mm][jj][0][0] == 'q':
                            elm_coe_out = 1
                        else:
                            elm_coe_out = 2  # 'tsx'|'tdx'
                        ...
                        for gg_out in range(1, elm_coe_out + 1):
                            if element_lines_H[mm][jj][gg_out][1] == stratum_ndx:
                                ...
                                evi_break = False
                                for tt in reversed(list(range(len_H0))):
                                    c_hgt = diff_y_lines[tt]

                                    if element_lines_H[mm][tt][0][0] == 'q':
                                        elm_coe_in = 1
                                    else:
                                        elm_coe_in = 2  # 'tsx'|'tdx'

                                    for gg_in in reversed(list(range(1, elm_coe_in + 1))):
            ...
            mean_effective_stress_atm = float(st_ES)
...
w_multiplier = float(input_dict[json_elem_dict["mesh"]][json_elem_dict["mesh_dow"]][json_elem_dict["mesh_dow_fm"]][
                         json_elem_dict["etc_hv"]])
h_multiplier = float(input_dict[json_elem_dict["mesh"]][json_elem_dict["mesh_dow"]][json_elem_dict["mesh_dow_fm"]][
                         json_elem_dict["etc_vh"]])
...
if mesh_downsampling:
    ...
    if isinstance(diff_x_lines, type(None)):
        ...
        print('ERROR: element belonging to last vertical polyline and described by nodes ' + str(
            f_elm_H[1][0][0][0]) + ', ' + str(f_elm_H[1][0][1][0]) + ', ' + str(f_elm_H[1][0][2][
                                                                                    0]) + ' is triangular. This should never happen, since the condition "... or a_ndx == 0 or a_ndx == len(y_arr_lines)-2" has been added in the code. Please use "ipython" to debug this!')
        sys_exit(1)
    if isinstance(diff_y_lines, type(None)):
        f_elm_H = element_lines_H
        print('ERROR: element belonging to last horizontal polyline and described by nodes ' + str(
            f_elm_H[1][0][0][0]) + ', ' + str(f_elm_H[1][0][1][0]) + ', ' + str(f_elm_H[1][0][2][
                                                                                    0]) + ' is triangular. Try to increment "distance_threshold_m" within inner-block "rearrange_nodes".')
        sys_exit(1)
    ...
    for ref_soil in [x + 1 for x in range(strata)]:
        for c_round in range(1, 8):
            ...
            element_lines_H, element_lines_H_mask, element_lines_array, \
                ...
                mesh_ds(element_lines_H, element_lines_H_mask, ref_soil, min_F_nodes, json_elem_dict, \
                        input_dict, w_fac_mul, h_fac_mul, dict_width_s, dict_height_s, diff_x_lines, diff_y_lines,
                        tri_extr)
...
if len(element_lines_write) > 99999:
    print(
        'ERROR: due to limit of five numeric digits to be read by QUAD4M for element-IDs (it is "%5i"), maximum allowed number of elements is fixed to 99999 (' + str(
            len(element_lines_write)) + ' have been generated instead), aborting script.')
    sys_exit(1)
...
element_lines_t = list(map(list, list(zip(*element_lines))))
...
e_l_9_w = list(map(list, list(zip(*e_l_9))))

# --------

element_lines_w = list(map(list, list(zip(*element_lines_write))))
nodal_points_pre = []
...
modp_c_X = input_dict[json_elem_dict["modp"]][json_elem_dict["modp_acc"]][json_elem_dict["modp_acc__X"]][
    json_elem_dict["etc_i"]]
modp_c_Y = input_dict[json_elem_dict["modp"]][json_elem_dict["modp_acc"]][json_elem_dict["modp_acc__Y"]][
    json_elem_dict["etc_i"]]
modp_cXY = input_dict[json_elem_dict["modp"]][json_elem_dict["modp_acc"]][json_elem_dict["modp_acc_XY"]][
    json_elem_dict["etc_i"]]
...
modp_d_X = input_dict[json_elem_dict["modp"]][json_elem_dict["modp_acc"]][json_elem_dict["modp_acc__X"]][
    json_elem_dict["etc_g"]]
modp_d_Y = input_dict[json_elem_dict["modp"]][json_elem_dict["modp_acc"]][json_elem_dict["modp_acc__Y"]][
    json_elem_dict["etc_g"]]
modp_dXY = input_dict[json_elem_dict["modp"]][json_elem_dict["modp_acc"]][json_elem_dict["modp_acc_XY"]][
    json_elem_dict["etc_g"]]
if modp_d_X:
...
if len(nodal_points_write) > 99999:
    print(
        'ERROR: due to limit of five numeric digits to be read by QUAD4M for node-IDs (it is "%5i"), maximum allowed number of nodes is fixed to 99999 (' + str(
            len(element_lines_write)) + ' have been generated instead), aborting script.')
    sys_exit(1)
...
plt, plt_ax = post_plot(plt, plt_ax, no_bc_array_1, no_bc_array_2, no_bc_array_3, \
                        no_bc_array_4, no_out_array_1, no_out_array_2, no_out_array_3, leg_fontsize,
                        ax_markersize * 2.0)
# #############################
plt, plt_ax = finalize_plot(plt, plt_ax, ax_linewidth, ax_fontsize)
plt_ax.tick_params(labelsize=ax_fontsize * 0.85)
...
if save_png: figure.savefig((mesh_nam[:-3] + 'png').replace(outfold, png_outfold), format='png', dpi=600,
                            bbox_inches='tight')
# #############################
...
xy_triang_DENS = list(map(list, list(zip(*z_triang))))[0]
xy_triang_DENS = [x / 1000.0 for x in xy_triang_DENS]
xy_triang_PO = list(map(list, list(zip(*z_triang))))[1]
xy_triang_GMX = list(map(list, list(zip(*z_triang))))[2]
xy_triang_GMX = [x / 1000.0 for x in xy_triang_GMX]
xy_triang_G = list(map(list, list(zip(*z_triang))))[3]
xy_triang_G = [x / 1000.0 for x in xy_triang_G]
xy_triang_XL = list(map(list, list(zip(*z_triang))))[4]
...
row04 = '{:10.4f}'.format(1) + '{:10.4f}'.format(0.65) + '{:10.1f}'.format(modp_rvp) + '{:10.1f}'.format(
    modp_rvs) + '{:10.1f}'.format(modp_rho)
...
row08 = '{:5.0f}'.format(lenwave) + '{:5.0f}'.format(wave_ltl) + '{:5.0f}'.format(wave_ftl) + '{:5.0f}'.format(
    wave_fts) + '{:5.0f}'.format(wave_lts) + '{:5.0f}'.format(modp_nmb) + '{:5.0f}'.format(2) + '{:5.0f}'.format(1)
row09 = '      DTEQ    EQMUL1    EQMUL2    UGMAX1    UGMAX2 HDRX HDRY NPLX NPLY   PRINPUT *** (5F10.0,4I5,F10.0) ***'
row10 = '{:10.5f}'.format(wave_tss) + '{:10.5f}'.format(1) + '{:10.5f}'.format(1) + '{:10s}'.format(
    '') + '{:10s}'.format('') + '{:5.0f}'.format(wave_hrx + 1) + '{:5.0f}'.format(wave_hry + 1) + '{:5.0f}'.format(
    1) + '{:5.0f}'.format(1) + '{:10.5f}'.format(wave_prp)
...
for stratum_ndx in [x + 1 for x in range(strata)]:
    ...
    try:
        ...
        plt_ax = plt.gca()
        c_plt_soi_gg = plt_ax.plot(list(zip(*soi_gg))[0], list(zip(*soi_gg))[1], color='black', marker='o',
                                   linewidth=c_linewidth, markersize=ax_markersize)
        plt.xscale('log')
        plt.title(c_na, fontsize=ax_fontsize, fontweight="bold")
        plt.grid(True, linestyle=':', dash_capstyle='round', dash_joinstyle='round', linewidth=ax_linewidth,
                 color=[0.5, 0.5, 0.5])
        plt.grid(True, axis='x', which='minor', linestyle=':', dash_capstyle='round', dash_joinstyle='round',
                 linewidth=ax_linewidth * 0.7, color=[0.8, 0.8, 0.8])
        plt.box(True)
        plt.ylim([0, 1.05])
        plt.xlim([0.0001, 1])
        ...
        if save_png: figure.savefig((f_nam[:-3] + 'png').replace(outfold, png_outfold), format='png', dpi=600,
                                    bbox_inches='tight')

        plt.ylabel('Damping, $D$ [%]', fontsize=ax_fontsize)
        for handle in c_plt_soi_gg: handle.remove()
        plt_ax.plot(list(zip(*soi_xl))[0], list(zip(*soi_xl))[1], color='black', marker='o', linewidth=c_linewidth,
                    markersize=ax_markersize)
        if max(list(zip(*soi_xl))[1]) < 30: plt.ylim([0, 30])
...
for cc_s in c_s:
    BO_fid.write('{:10.3f}'.format(cc_s[0][0]) + '{:10.3f}'.format(cc_s[0][1]) + '{:10.3f}'.format(
        cc_s[1][0]) + '{:10.3f}'.format(cc_s[1][1]) + '\n')
BO_fid.write('\n')
for cc_s in c_s_9:
    BO_fid.write('{:10.3f}'.format(cc_s[0][0]) + '{:10.3f}'.format(cc_s[0][1]) + '{:10.3f}'.format(
        cc_s[1][0]) + '{:10.3f}'.format(cc_s[1][1]) + '\n')
...
for cc_ndx in range(len(c__X_MP.geoms)):
    BO_fid.write('{:10.3f}'.format(c__X_MP.geoms[cc_ndx].x) + '{:10.3f}'.format(c__X_MP.geoms[cc_ndx].y) + '\n')
BO_fid.write('\n')
BO_fid.write('{:10.3f}'.format(-999.999) + '{:10.3f}'.format(-999.999) + '\n')
for cc_ndx in range(len(c__Y_MP.geoms)):
    BO_fid.write('{:10.3f}'.format(c__Y_MP.geoms[cc_ndx].x) + '{:10.3f}'.format(c__Y_MP.geoms[cc_ndx].y) + '\n')
BO_fid.write('\n')
BO_fid.write('{:10.3f}'.format(-999.999) + '{:10.3f}'.format(-999.999) + '\n')
for cc_ndx in range(len(c_XY_MP.geoms)):
    BO_fid.write('{:10.3f}'.format(c_XY_MP.geoms[cc_ndx].x) + '{:10.3f}'.format(c_XY_MP.geoms[cc_ndx].y) + '\n')
BO_fid.close()
...
timeArr = np.array(list(range(0, len(waveXg)))) * wave_tss
...
therectF0 = patches.Rectangle(xy=(wave_fts * wave_tss, -wave_max * v003 / v001),
                              width=wave_lts * wave_tss - wave_fts * wave_tss, height=2 * wave_max * v003 / v001,
                              fill=False, linestyle='-', edgecolor='g', alpha=0.75, label='first iterations',
                              linewidth=c_linewidth)
therectF1 = patches.Rectangle(xy=(wave_fts * wave_tss, -wave_max * v003 / v001),
                              width=wave_lts * wave_tss - wave_fts * wave_tss, height=2 * wave_max * v003 / v001,
                              fill=False, linestyle='-', edgecolor='g', alpha=0.75, label='first iterations',
                              linewidth=c_linewidth)
therectL0 = patches.Rectangle(xy=(wave_ftl * wave_tss, -wave_max * v002 / v001),
                              width=wave_ltl * wave_tss - wave_ftl * wave_tss, height=2 * wave_max * v002 / v001,
                              fill=False, linestyle='-', edgecolor='r', alpha=0.75, label='last iteration',
                              linewidth=c_linewidth)
therectL1 = patches.Rectangle(xy=(wave_ftl * wave_tss, -wave_max * v002 / v001),
                              width=wave_ltl * wave_tss - wave_ftl * wave_tss, height=2 * wave_max * v002 / v001,
                              fill=False, linestyle='-', edgecolor='r', alpha=0.75, label='last iteration',
                              linewidth=c_linewidth)
...
plt_ax[0].annotate('', xy=(0.02, 0.55), xycoords='figure fraction', xytext=(0.02, 0.45),
                   arrowprops=dict(arrowstyle="-", color='w'))
plt_ax[0].annotate('', xy=(0.98, 0.55), xycoords='figure fraction', xytext=(0.98, 0.45),
                   arrowprops=dict(arrowstyle="-", color='w'))
...
if save_png: figure.savefig((wav_nam[:-3] + 'png').replace(outfold, png_outfold), format='png', dpi=600,
                            bbox_inches='tight')
